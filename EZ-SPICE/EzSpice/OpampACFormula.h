...
public:
	...
	BOOL DivideLeftCircuit(list<IPCData> &LeftCircuitDataList, list<IPCData> &LeftRealDatalist, list<CBaseElement*> &Provlist);	//Opamp를 기준으로 왼쪽 회로 분리 및 수행
	BOOL DivideRightCircuit(list<IPCData> LeftCircuitDataList, list<IPCData> &Datalist, list<CBaseElement*> &Provlist);			//Opamp를 기준으로 오른쪽 회로 분리 및 수행
